...
export class cmAttributeControls extends SvgGroupElement {
  constructor(parent, name, isVertical, width, height, attributeIndex) {
    super(parent);
    ...
    if (!isVertical) {
      cmAttributeControls.createLabel(group, name, isVertical, width);
    }

    this.createInteractionRect(group, isVertical, width, height);
  }

  static createLabel(group, name, isVertical, width) {
    if (!isVertical) {
      group.append("text")
  ...
  createInteractionRect(group, isVertical, width, height) {
    ...
    if (!isVertical) {
      ...
      this.controls.append("i")
        ...
        .attr("float", "left")
        .on("click", function () {
          console.log("hello " + self.name + " sorted");
        });
